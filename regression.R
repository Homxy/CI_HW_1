...
# Load your dataset
my_data <- read.csv("D:\\MyWorkSpace\\dataset\\flood.csv")
...
X <- as.matrix(scaled_data[, 1:8])
y <- as.matrix(scaled_data[, 9])
...
predicted_outputs <- numeric(length(y)) # Changed to numeric
...
for (k in 1:k_folds) {
  ...
  predicted_outputs[test_indices] <- predicted_output # Store the predictions
...
plot(test_length, my_data[, 9], col = "red", type = "l", xlab = "Index", ylab = "Value", main = "Actual vs. Predicted Values")
lines(test_length, predicted, col = "blue")
